...
//(~9.10^-3 Hz wave -- 31/3200 -- required by the tone function)
...
void Pulse::start_burst(unsigned long n, unsigned long freq){
  ...
  if((freq <= MAX_FREQ) && (freq >= MIN_FREQ) && (n > 0)){
    //set current mode
    this->_mode = 2;
    //compute when the burst should end (current time + number of period * period)
    this->_time_end = micros() + (unsigned long)((double)n/(double)freq*1000000.0);
    //start output signal generation
    tone(this->_pin, freq);
  #ifdef DEBUG
    ...
    if(buffer.length() > 0){
      ...
      switch(buffer[0]){
        ...
        case 'C': //Continuous
          ...
          break;
        case 'B': //Burst
          //the comma separates the number of pulses and the frequency
